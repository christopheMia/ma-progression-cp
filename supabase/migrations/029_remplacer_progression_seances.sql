-- Tache 4b : la derniere porte fermee entre l'IA et la base.
--
-- La migration 028 a pose la colonne `progression.seances`. Mais
-- `remplacer_progression` nomme ses colonnes une par une : une ligne portant
-- des seances etait acceptee sans erreur, et ses seances jetees en silence.
-- C'est le defaut le plus couteux de la famille, parce qu'il ne casse rien :
-- l'import reussit, l'enseignante ne voit aucun message, et le cahier journal
-- reste vide.
--
-- Signature INCHANGEE (uuid, uuid, text, integer[], jsonb, boolean) : le code
-- appelant ne change pas d'appel, et `enregistrer_source_progression`
-- (migration 016) continue de passer `p_lignes` sans en connaitre le contenu.
--
-- Trois ajouts seulement, par rapport a la migration 014 :
--   1. `seances` dans la liste des colonnes de l'insert ;
--   2. `coalesce(x.seances, '[]'::jsonb)` dans le select, parce que la colonne
--      est `not null` et qu'une ligne sans le champ rendrait `null` ici ;
--   3. `seances jsonb` dans la definition du jsonb_to_recordset.
--
-- La branche `p_sync_semaines` n'est PAS touchee : elle ecrit dans la table
...
begin
  ...
  insert into progression (
    class_id, methode_id, matiere, numero, items, pages, mots_exemple, seances
  )
  select
    p_class_id,
    p_methode_id,
