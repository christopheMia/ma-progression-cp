...
-- Critères d'observation personnalisés et résultats par élève (migration 018).
...
do $$ begin
  alter table public.criteres_observation
    add constraint criteres_observation_libelle_non_vide
    check (length(trim(libelle)) > 0);
exception when duplicate_object then null; end $$;
do $$ begin
  alter table public.criteres_observation
    add constraint criteres_observation_unique
    unique (semaine_id, matiere, notion, libelle);
exception when duplicate_object then null; end $$;
do $$ begin
