...
export default function SetupPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-rose-800 mb-2">Configuration de ma classe</h1>
        <div className="flex gap-2">
          {stepTitles.map((title, i) => (
            <div key={i} className={`flex-1 h-2 rounded-full ${i + 1 <= step ? 'bg-rose-600' : 'bg-gray-200'}`} />
          ))}
        </div>
